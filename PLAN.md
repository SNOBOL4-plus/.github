...
## Sprint 17 — sno_exit() on Failure Paths
**Recorded 2026-03-10**

FuncEmitter never calls sno_exit(). Every sno_enter() allocates a frame;
none are freed on failure. For shallow patterns (Sprints 0-13): fine.
For recursive grammars with deep backtracking: arena fills exponentially.

Fix: at {name}_match_fail in FuncEmitter, call sno_exit() for every
frame allocated in that call path before returning SNO_EMPTY.

Alternative: FlatLoopEmitter (separate backtrack stack) makes this moot.
The sno_exit() fix is tactical; FlatLoopEmitter is architectural.

Evidence: 6 failures in Sprint 16, all paren depth 4+, all arena overflow.
Python generators are lazy (no frame until yield). C FuncEmitter is eager.

Sprint 17 deliverable: sno_exit() at match_fail. Oracle: 609/609.
...
to talk about. I did this technique before when I wrote Flash BASIC for Richard Pick
...
writing Flash BASIC at Pick Systems (with Richard Pick and David Zigray). Exhaustive
...
| 2026-03-10 | **Sprint 19 — The Worm Unchained.** How long does the worm get? `gen_term()` is infinite. It generates expressions of growing structural complexity: 1, 6, 15, 29, 49, 76, 111, 155, 209, 274, 351, 441... doubling roughly every 8 steps. Sprint 19 runs the worm to 1000 systematic cases + 500 random cases. Longest correct expression: **1,204 characters**. Total: **1500/1500 passed** in 11.5s. Zero failures. The compiled evaluator is proven correct at all token lengths. Arena scales. Value stack holds. Floor division correct. Backtracking engine correct at all grammar depths. Commit `c5e5c6a`. **Next: Beautiful.sno — Sprint 20 acceptance test.** |
| 2026-03-10 | **Sprint 18 — The Self-Hosting Moment.** Value stack added to runtime (`sno_vpush/sno_vpop/sno_vpeek/sno_vreset/sno_vdepth`, 256-deep, `runtime.h` + `runtime.c`). `build_expr_evaluator.py` emits a compiled expression evaluator: same grammar as Sprint 16 but every pattern pushes its computed integer value. Binary takes expression on `argv[1]`, prints result. Key bugs found and fixed by the worm: (1) vstack underflow on beta paths — fixed by storing `saved_left` in frame struct and re-pushing on beta; (2) C truncation vs Python floor division — fixed with `_floor_div()` macro matching Python `//` semantics. Worm oracle: gen_term 100/100, rand 500/500, arithmetic 20/20. **620/620. Zero failures.** The compiled evaluator agrees with the Python reference on every generated expression. The compiler can replace its own reference implementation. SNOBOL4-tiny commit `0cbf100`. **Next: Sprint 19 — EVAL (the hard one).** |
| 2026-03-10 | **Sprint 17 — sno_exit() arena fix.** One line change: `sno_exit((void **)zz)` at `{name}_match_fail` in FuncEmitter. Arena freed on failure — no more frame accumulation across failed backtrack paths. Sprint 16 oracle: 603/609 → **609/609**. Zero regressions: Sprint 14 7/7, Sprint 15 580/580, Sprint 16 609/609. Commit `0050c7b`. **Next: Sprint 18 — evaluate() round-trip.** |
| 2026-03-10 | **Sprint 16 — Bridge: Expressions.py to Compiled IR.** Translated parse_item/element/factor/term/expression from Expressions.py generators to IR nodes — mechanical and exact. build_expr_parser.py emits C via emit_c.py. Worm: gen_term 100/100, rand 494/500, negatives 9/9. 603/609 total. 6 failures: paren depth 4+ triggers exponential backtracking; FuncEmitter never calls sno_exit() so arena fills. Arena raised to 4MB. Sprint 17 fix: sno_exit() on failure paths. Commit 7278378. |
...
**Precedent**: Pick Systems — Lon built this exact architecture with Richard Pick and David Zigray.
...
## IDEA — Tie COMM Into &TRACE / &STCOUNT: The Canonical Hook
**Origin**: Lon Cherryholmes, 2026-03-10
**Status**: RECORD THIS. This simplifies the entire monitor architecture.

### The Discovery

CSNOBOL4 and SPITBOL already have a per-statement execution hook built in.
It is not an afterthought — it is the core interpreter loop. From `v311.sil`:

```
       MOVA    STNOCL,XCL         Update &STNO        ← fires every statement
       INCRA   EXNOCL,1           Increment &STCOUNT  ← fires every statement
       ACOMPC  TRAPCL,0,,RTNUL3  Check &TRACE        ← if &TRACE set, call TRPHND
       LOCAPT  ATPTR,TKEYL,STNOKY Check for STNO trace
       RCALL   ,TRPHND,ATPTR      Perform trace       ← THIS is the hook
```

`&STNO` is the current statement number. `&STCOUNT` is the total executed.
`&TRACE` is the flag. `TRPHND` is the trace handler — a dispatch point that
fires on every statement, every variable assignment, every function call/return.

**The SNOBOL4 TRACE() function taps directly into this mechanism.**

The standard usage:
```snobol4
        TRACE('&STNO', 'KEYWORD')    :S(CONTINUE)
```
After this call, every statement execution fires a trace output to `&ERROUT`
showing the statement number. That IS the COMM stream — it already exists,
it is already per-statement, and it already fires in both oracle and binary.

### What This Means for the Monitor Architecture

**We don't need to instrument `snobol4.c` or `emit_c_stmt.py` at all
for the oracle side.** The oracle trace is generated by adding three lines
to the top of `beauty_run.sno`:

```snobol4
*-- Monitor instrumentation (remove for production)
        TRACE('&STNO','KEYWORD')
        TRACE('snoLine','VALUE')
        TRACE('snoSrc','VALUE')
```

This produces a trace stream on `&ERROUT` (stderr) in the canonical
CSNOBOL4 trace format:
```
** &STNO = 1
** &STNO = 2
** snoLine = "  OUTPUT = x"
...
**For the compiled binary side**, we implement the same `sno_comm_*` hooks
in `snobol4.c` — but now we know EXACTLY what format to match, because
the oracle's format is defined by the SNOBOL4 standard TRACE output.

The COMM format in our runtime mirrors the TRACE format precisely:
```
STNO 613
VAR snoLine "  OUTPUT = x"
STNO 614
```

### The Deep Insight: TRACE IS the Spec

The SNOBOL4 TRACE() function is not just a debugging tool — it is a
**standard, portable, oracle-verified event stream**. Every conforming
SNOBOL4 implementation must produce the same trace events for the same
program. CSNOBOL4 and SPITBOL both support it.

This means:
- The oracle trace format is not something we invent — it is already specified
- Any SNOBOL4 program can be traced without modifying the runtime
- The trace output IS the execution semantics, made visible
- Our compiled binary's COMM stream must match this format to be correct

**When our binary's COMM stream matches the CSNOBOL4 TRACE stream exactly
(modulo the ignore list), the binary is semantically equivalent to the oracle.**

That is not just a debugging technique. That is a correctness proof.

### Revised Three-Component Architecture

```
COMPONENT 1 — ORACLE TRACE (zero code, just SNOBOL4):
  beauty_run_traced.sno = beauty_run.sno + TRACE('&STNO','KEYWORD') header
  snobol4 -f -P256k beauty_run_traced.sno < beauty_run.sno 2> oracle_trace.txt

COMPONENT 2 — BINARY COMM (instrument snobol4.c + emit_c_stmt.py):
  sno_comm_stno(N)   — emitted at each statement label in beautiful.c
  sno_comm_var(name, val) — emitted inside sno_var_set() for watched vars
  ./beautiful < beauty_run.sno 2> binary_trace.txt

COMPONENT 3 — DIFF MONITOR (diff_monitor.py, ~30 lines):
  compare oracle_trace.txt vs binary_trace.txt line by line
  skip lines matching monitor_ignore.json
  stop at FIRST diff, print it
```

### The &STCOUNT / &STLIMIT Connection

`&STCOUNT` is a free binary search tool. Because `&STLIMIT` stops execution
at a given statement count:

```snobol4
        &STLIMIT = 500   * stop after 500 statements
```

You can binary-search to the exact statement where the oracle and binary diverge:
- Run both with `&STLIMIT = 1000` — do they agree up to statement 1000?
- If yes, try 2000. If no, try 500.
- Bisect until you have the exact statement count where divergence begins.

This is **binary search debugging** — O(log N) instead of O(N).
For `beauty_run.sno` which runs ~649 output lines × ~10 statements each
≈ 6490 statements, bisection finds the bug in ~13 iterations.

The monitor automates this: if the first diff is at STNO 4821, it knows
to re-run with `&STLIMIT = 4821` to reproduce exactly, and with
`&STLIMIT = 2410` to check if the bug is in the first or second half.

### Action Items (revised)
- [ ] Add TRACE header to `beauty_run_traced.sno` — generate oracle trace
- [ ] Add `sno_comm_stno(N)` to `emit_c_stmt.py` — one call per statement label
- [ ] Add `sno_comm_var(name, val)` to `sno_var_set()` for watched variables
- [ ] Write `diff_monitor.py` — compare two trace files, stop at first diff
- [ ] Build ignore list from trivial input calibration run
- [ ] Run on `beauty_run.sno` — find the hang STNO
- [ ] Binary search with `&STLIMIT` to isolate — or the hang STNO IS the bug location
- [ ] Report first diff here — Claude diagnoses and fixes

### &STCOUNT Is Always Free
Per P3 note in Outstanding Items: on modern hardware, `&STCOUNT` increment
