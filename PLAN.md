...
|---------|--------|------|----------------|
| **TINY backend** | `asm-backend` B-219 — M-EMITTER-NAMING ⚠ WIP: source names done, C merged; JVM+NET generated labels still need α/β/γ/ω port suffixes | `5999162` B-219 | M-EMITTER-NAMING |
...
| **M-EMITTER-NAMING** | All four emitters in one file each ✅; source names canonical ✅; REMAINING: JVM generated Byrd port labels must use α/β/γ/ω suffixes (currently `Jn%d_lit_ok`, `Jn%d_seq_mid` etc — 65 sites); NET generated labels must use α/β/γ/ω suffixes (currently `Nn%d_nam_ok`, `Nn%d_seq_m%d` etc — 22 sites). Fires when: grep of any JVM/NET `.j`/`.il` output shows `_α`/`_β`/`_γ`/`_ω` port labels AND 106/106 C + 26/26 ASM hold. | ⚠ `5999162` B-219 |
