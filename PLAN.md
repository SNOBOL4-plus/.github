...
### CSNOBOL4 Syntax Tables — Authoritative Reference for lex.c

**Recorded Session 53. Lon's note: v311.sil has 256-byte syntax tables. Also the IBM 360
version (EBCDIC), and many ports of CSNOBOL4 carry them. Use these as the reference
for the hand-rolled lexer character classification.**

#### What These Are

CSNOBOL4's lexer is built around the **STREAM instruction** from the original SIL
(SNOBOL Implementation Language). STREAM walks a string character by character through
a 256-entry dispatch table. Each entry is an action index (0 = CONTIN, i.e. keep scanning;
nonzero = look up action struct). The action struct says: PUT a token type, take an ACTION
(STOP, STOPSH, ERROR, or GOTO another table).

This is exactly the architecture we want for `lex.c` — and the tables are pre-computed
and available in the CSNOBOL4 source.

#### Files in `snobol4-2.3.3/` tarball (already extracted at `/tmp/snobol4-2.3.3/`)

| File | What it is |
|------|-----------|
| `syntax.tbl` | **Source of truth** — human-readable state machine (FORs, ELSEs, GOTOs). 336 lines. |
| `gensyn.sno` | SNOBOL4 program that compiles `syntax.tbl` → `syn.c` + `syn.h`. Character class assignments are here. |
| `syn.c` | **Pre-generated C** — all `struct syntab` tables with their 256-byte `chrs[]` arrays. |
| `syn.h` | `extern struct syntab BIOPTB;` etc. — all table names. |
| `include/syntab.h` | `struct syntab { const char *name; char chrs[256]; const struct acts *actions; }` |
| `lib/stream.c` | The 80-line C implementation of STREAM — just `tp->chrs[*cp]` dispatch in a loop. |

#### The Character Classes (from `gensyn.sno`)

```
LETTER      = A-Z a-z (plus high bytes 129+ treated as alpha for UTF-8)
NUMBER      = 0-9
...
NOTSYM      = '~' '\'
QUESYM      = '?'
CMT         = '*' '#' '|' ';' '!'   (first char of line = comment)
CTL         = '-'                    (control card)
CNT         = '+' '.'               (continuation line)
FGOSYM      = 'F' 'f'               (Failure goto)
SGOSYM      = 'S' 's'               (Success goto)
EOS         = ';'                    (end of statement, in-line)
EXP         = 'E' 'e'               (exponent in float literal)
BREAK       = '.' '_'               (valid in identifiers)
TERMINATOR  = COMMA + BLANK + EOS + RIGHTBR + RIGHTPAREN
```

#### The Key State Machines (from `syntax.tbl`)

| Table | Purpose |
|-------|---------|
| `CARDTB` | First char of line: CMT→comment, CTL→control, CNT→continuation, else→new stmt |
| `ELEMTB` | Start of expression element: NUMBER→integer, LETTER→variable, SQUOTE/DQUOTE→string literal, LEFTPAREN→nested |
| `VARTB` | Continue variable name: ALPHANUMERIC/BREAK→keep going, LEFTPAREN→function call (FNCTYP), LEFTBR→array (ARYTYP), TERMINATOR→stop |
| `INTGTB` | Continue integer: NUMBER→keep, TERMINATOR→emit int, DOT→switch to float, EXP→exponent |
| `FLITB` | Continue float fraction: NUMBER→keep, TERMINATOR→stop, EXP→exponent |
| `SQLITB`/`DQLITB` | String literal: matching quote→stop, else→continue |
| `BIOPTB` | Binary operator: `+`→ADD, `-`→SUB, `.`→NAME, `$`→DOLLAR, `*`→MPYFN/STARTB (two-char `**`), `/`→DIV, `@`→AT, etc. |
| `UNOPTB` | Unary operator: same chars, different fn codes |
| `FRWDTB` | Skip blanks / classify delimiter: BLANK→skip, `=`→EQTYP, `)`→RPTYP, `]`→RBTYP, `,`→CMATYP, `:`→CLNTYP, EOS→stop |
| `GOTOTB` | Goto field: `S`/`s`→success, `F`/`f`→failure, `(`→unconditional |
| `LBLTB`/`LBLXTB` | Label scanning |
| `STARTB` | Disambiguate `*` vs `**`: `*`→exponent (two-star), BLANK→stop (unary star) |

#### The EBCDIC Angle

The original IBM 360 MAINBOL used EBCDIC. The `syn.c` tables are **ASCII** (generated by
`gensyn.sno` using `&ALPHABET` of the host). The character class assignments in `gensyn.sno`
are by character value (`CLASS<"SQUOTE"> = "'"` etc.), so `gensyn.sno` run on an EBCDIC
host would produce an EBCDIC-indexed `syn.c`. For SNOBOL4-tiny we only care about ASCII,
but the **structure** is identical — and the IBM 360 SIL `v311.sil` (pre-CSNOBOL4) would
have hand-assembled versions of these same tables in `DEFINE`/`BYTE` blocks.

#### How to Use This in lex.c

**Option A — table-driven (faithful to CSNOBOL4)**:
Copy `syn.c`/`syntab.h` verbatim into `src/snoc/`. Write `lex.c` to call `stream()` from
`lib/stream.c`. Gives you CSNOBOL4-identical tokenization for free, including edge cases
(e.g. `**` as exponent, `._` as BREAK chars in identifiers, `;` as in-line EOS).

**Option B — direct switch/lookup (simpler build)**:
Build a single `static unsigned char sno_charclass[256]` array in `lex.c` using the same
class assignments as `gensyn.sno`. Write `next_token()` as a switch on that array. Same
semantics, no external dependency, fits in ~50 lines.

**Recommendation**: Option B. We understand the classes fully now. The stream.c machinery
adds complexity without benefit — we don't need plugtb/clertb (runtime table patching used
by SNOBOL4+'s SPAN/BREAK builtins). A flat `sno_charclass[256]` + switch gives the same
classification with zero overhead.

**The VARTB disambiguation** is the key insight for `lex.c`:
- `LETTER (ALPHANUMERIC|BREAK)*` followed by `(` → token type FNCTYP (function call)
- `LETTER (ALPHANUMERIC|BREAK)*` followed by `[` → token type ARYTYP (array subscript)
- `LETTER (ALPHANUMERIC|BREAK)*` followed by anything else → VARTYP (plain identifier)

This is the token-level distinction our hand-rolled parser will use to know — at lex time,
