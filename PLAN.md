...
| D5 | SNOBOL4cython v2 repo destination | **New org repo** · Fold into SNOBOL4-python · Fold into SNOBOL4-tiny · Leave in zips | **OPEN — clarification in progress** | Lon has both zips locally. The question is which GitHub repo to push to so the 721-line C engine is not lost if local machine dies. Clarifying whether new `SNOBOL4-cython` org repo or subfolder in SNOBOL4-tiny. | Answer before end of session |
