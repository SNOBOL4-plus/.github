...
|---------|--------|------|----------------|
| **TINY backend** | `asm-backend` B-216 — M-EMITTER-NAMING ✅ complete: ASM/JVM/NET static internals prefix strip done; 106/106 C + 26/26 ASM hold; beauty_prog.s regenerated clean | `52baf6e` B-216 | M-ASM-RUNG8 |
...
| **M-EMITTER-NAMING** | All four emitters (C, ASM, JVM, NET) obey the naming law: (1) same concept = same root name in all four source files — `vars[]`, `nvar`, `var_register()`, `named_pats[]`, `named_pat_count`, `named_pat_register()`, `next_uid()`, `out` (FILE*), `emit_stmt()`, `emit_pat_node()`, `NamedPat`, `FnDef`, `DataType` — sugar (case/prefix/suffix) varies by context, root is invariant; (2) generated output uses α/β/γ/ω on every Byrd port label in every backend where the target allows Unicode, spelled-out alpha/beta/gamma/omega where it does not — ASM ✅ already uses Greek; JVM and NET label schemes must be updated to carry the Greek port suffix; (3) entry points `asm_emit`/`jvm_emit`/`net_emit`/`c_emit` consistent (extern-visible, intentionally prefixed); (4) output macros `A()`/`J()`/`N()`/`C()` are the one intentional source-level difference. Remaining: strip per-backend prefixes from ASM/NET/JVM static internals; add α/β/γ/ω to JVM+NET generated labels. | ✅ `52baf6e` B-216 |
