...
| **TINY frontend** | `main` F-215 — duplicate session: F-214 had already fired M-PROLOG-WIRE-ASM ✅ + M-PROLOG-HELLO ✅ (`082141e`) before F-215 landed; stash dropped, no new code; OPEN BUG unchanged: call sites emit `pl_FUNCTOR_ARITY_r` but predicates defined as `pl_FUNCTOR_sl_ARITY_r`; fix: pass `"functor/arity"` through `pl_safe()` at every call site, drop `_%d_r` suffix | `082141e` F-214 | M-PROLOG-R1 |
