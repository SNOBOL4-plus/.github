...
## ⛔ ARTIFACTS — Five canonical demo programs tracked every session

All five demo programs and their data live in `demo/` — single source of truth.
Every session that changes `emit_byrd_asm.c`, `snobol4_asm.mac`, or any `.sno → .s` path
MUST regenerate all five canonical ASM artifacts. **Update only if changed — do not commit unchanged files.**

| Artifact | Source | Status |
|----------|--------|--------|
| `artifacts/asm/beauty_prog.s` | `demo/beauty.sno` | ✅ assembles clean |
| `artifacts/asm/samples/roman.s` | `demo/roman.sno` | ✅ assembles clean |
| `artifacts/asm/samples/wordcount.s` | `demo/wordcount.sno` | ✅ assembles clean |
| `artifacts/asm/samples/treebank.s` | `demo/treebank.sno` | ✅ assembles clean |
| `artifacts/asm/samples/claws5.s` | `demo/claws5.sno` | ⚠️ ~95%: 3 undef β labels (NRETURN fns) |

```bash
INC=demo/inc

./sno2c -asm -I$INC demo/beauty.sno    > /tmp/beauty_new.s
./sno2c -asm -I$INC demo/roman.sno     > /tmp/roman_new.s
./sno2c -asm -I$INC demo/wordcount.sno > /tmp/wordcount_new.s
./sno2c -asm -I$INC demo/treebank.sno  > /tmp/treebank_new.s
./sno2c -asm -I$INC demo/claws5.sno    > /tmp/claws5_new.s
